...
import { buildTaskGenerationMessages, buildAddMoreTasksMessages, buildCompletionCheckMessages, detectOutcomeNarration } from './prompts.js';
import { parseTasksFromResponse, parseCompletionFromResponse } from './parsers.js';
import { getProfileApi, injectSteeringPrompt, removeSteeringPrompt, showToast, injectSoftNudge, injectActionNudge } from './services.js';
...
export async function generateStorySteps(storyGoal) {
    ...
    try {
        ...
        const messages = buildTaskGenerationMessages(context, storyGoal.trim(), settings.numberOfSteps || 5, settings.generateSubtasks);
...
export async function addMoreStorySteps(numberOfNewSteps, customGoal) {
    ...
    try {
        const messages = buildAddMoreTasksMessages(context, storyData.storyGoal, storyData.storySteps, numberOfNewSteps, customGoal, settings.generateSubtasks);
...
export async function checkStepCompletion() {
    ...
    showToast('Check', `"${task.title}"`, 'info');
    ...
    try {
        ...
        if (cr.completed) {
            ...
            if (next >= storyData.storySteps.length) {
                ...
                showToast('Done', `"${task.title}"`, 'success');
            }
        } else {
            ...
            if (storyData.checkAttempts >= maxAttempts) {
                ...
                if (next >= storyData.storySteps.length) {
                    ...
                    storyData.isActive = false;
                    removeSteeringPrompt();
                    showToast('Force Done', `"${task.title}"`, 'info');
                } else {
                    storyData.currentStepIndex = next;
                    if (settings.autoInject) injectSteeringPrompt(context, settings);
                    showToast('Force Done', `"${task.title}"`, 'info');
                ...
                if (settings.autoInject) injectSteeringPrompt(context, settings);
                showToast('Not Done', `"${task.title}" (${storyData.checkAttempts}/${maxAttempts})`, 'info');
            }
        }
...
export async function onAIMessage() {
    ...
    const nudgeThreshold = Math.ceil(checkInterval / 2);

    if (storyData.aiMessagesSinceCheck === nudgeThreshold) {
        const task = storyData.storySteps[storyData.currentStepIndex];
        if (task) injectSoftNudge(context, task);
    }

    const chat = context.chat || [];
    const lastAIMsg = [...chat].reverse().find(m => !m.is_user);
    if (lastAIMsg && detectOutcomeNarration(lastAIMsg.mes || '')) {
        injectActionNudge(context);
    }
