export function parseTasksFromResponse(responseText) {
    ...
    if (parsed) {
        ...
        if (tasks) {
            return {
                tasks: tasks.map((t, i) => {
                    if (typeof t === 'string') return { title: `Task ${i + 1}`, description: t, npcs: [], subtasks: [] };
                    const subtasks = Array.isArray(t.subtasks)
                        ? t.subtasks.filter(s => typeof s === 'object' && s !== null).map(s => ({
                            title: s.title || 'Sub-step',
    ...
    if (taskLines.length > 0) {
        const tasks = taskLines.map((l, i) => {
            ...
            return { title: `Task ${i + 1}`, description: text, npcs: [], subtasks: [] };
        });
