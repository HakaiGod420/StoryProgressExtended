...
import { buildGoalsSummaryText, buildSteeringPromptText, buildSoftNudgeText, buildActionNudgeText } from './prompts.js';
...
export function removeSteeringPrompt() {
    ...
    try { ctx.setExtensionPrompt(EXTENSION_PROMPT_KEY_ACTION, '', 0, 0, false, 0); } catch { /* */ }
}

export function injectSoftNudge(context, task) {
    if (!context || typeof context.setExtensionPrompt !== 'function') return;
    if (!task) return;
    const nudgeText = buildSoftNudgeText(task);
    try {
        context.setExtensionPrompt(EXTENSION_PROMPT_KEY_NUDGE, nudgeText, PROMPT_POSITION_AFTER, PROMPT_DEPTH, true, PROMPT_ROLE_SYSTEM);
    } catch { /* */ }
}

export function injectActionNudge(context) {
    if (!context || typeof context.setExtensionPrompt !== 'function') return;
    const nudgeText = buildActionNudgeText();
    try {
        context.setExtensionPrompt(EXTENSION_PROMPT_KEY_ACTION, nudgeText, PROMPT_POSITION_AFTER, PROMPT_DEPTH, true, PROMPT_ROLE_SYSTEM);
    } catch { /* */ }
}
